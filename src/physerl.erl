    ...
    case parse_quantity(Str) of
        {ok, {quantity, Magnitude, Units}} ->
            case convert_units(Units, Magnitude) of
                {ok, ConvertedUnits} -> {ok, ConvertedUnits};
                Error -> Error
            end;
        Error -> Error
    end.

-spec add({float(), physerl_si:base()}, {float(), physerl_si:base()}) -> {ok, {float(), physerl_si:base()}} | {error, incompatible_units}.
add({Mag1, Units1}, {Mag2, Units2}) ->
    case units_equivalent(Units1, Units2) of
        true -> {ok, {Mag1 + Mag2, Units1}};
        false -> {error, incompatible_units}
    end.

-spec sub({float(), physerl_si:base()}, {float(), physerl_si:base()}) -> {ok, {float(), physerl_si:base()}} | {error, incompatible_units}.
...
-spec root({float(), physerl_si:base()}, integer()) -> {ok, {float(), physerl_si:base()}} | {error, negative_exponent}.
root({Magnitude, Units}, Root) when Root > 0 ->
    case lists:all(fun({_Key, Exponent}) -> Exponent rem Root =:= 0 end, maps:to_list(Units)) of
        true ->
...
%--- Helpers ------------------------------------------------------------------
-spec convert_units([{atom() | nil, atom(), integer() | nil}], float()) -> {ok, {float(), physerl_si:base()}} | {error, term()}.
convert_units(UnitList, Magnitude) ->
    foldl_combine_units(UnitList, #{}, Magnitude).

-spec foldl_combine_units([{atom() | nil, atom(), integer() | nil}], physerl_si:base(), float()) -> {ok, {float(), physerl_si:base()}} | {error, term()}.
foldl_combine_units([], AccBase, Magnitude) -> {ok, {Magnitude, AccBase}};
...
-spec convert_unit({atom() | nil, atom(), integer() | nil}, float()) -> {ok, {float(), physerl_si:base()}} | {error, term()}.
convert_unit({PrefixSym, UnitSym, Power}, Magnitude) ->
    case find_unit(UnitSym) of
        {ok, #unit{base = Base, factor = Factor}} ->
            case (PrefixSym) of
                nil -> PrefixVal = 1.0;
                _ -> case find_prefix(PrefixSym) of
                         {ok, #prefix{val = Val}} -> PrefixVal = Val;
                         _ -> PrefixVal = 1.0
                     end
            end,
            PowerToUse = case Power of
                            nil -> 1;
                            _ -> Power
                         end,
...
tokens(Str) ->
    case physerl_lexer:string(Str) of
        ...
        {error,{_, physerl_lexer, {_, Reason}}, _} -> {error, Reason}
    end.

-spec parse({ok, list()} | {error, term()}) -> {ok, term()} | {error, {syntax_error, term()}}.
parse({ok, Tokens}) ->
    case physerl_parser:parse(Tokens) of
        {ok, Terms} -> {ok, Terms};
        {error, {T, physerl_parser, _}} -> {error, {syntax_error, T}}
    end;
